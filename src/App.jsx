...
export default function App() {
  ...
  const [lang, setLang] = useState(() => localStorage.getItem('statai-lang') || 'ar');
  ...
  const handleSaveLang = (code) => {
    setLang(code);
    localStorage.setItem('statai-lang', code);
  };
  ...
  return (
    <div dir="rtl">
      ...
      {screen === 'auth' && (
        <AuthScreen onLogin={go('projects')}/>
      )}
      {screen === 'projects' && (
        <ProjectsScreen onNew={go('upload')} onOpen={go('plan')}/>
      )}
      {screen === 'upload' && (
        <UploadScreen onNext={go('plan')} onBack={go('projects')}/>
      )}
      {screen === 'plan' && (
        <PlanScreen onNext={go('results')} onBack={go('upload')}/>
      )}
      {screen === 'results' && (
        <ResultsScreen onBack={go('plan')}/>
      )}
    </div>
  );
}
