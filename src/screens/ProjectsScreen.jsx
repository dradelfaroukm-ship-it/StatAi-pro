...
const LEVEL_LABEL  = { basic: 'أساسي', medium: 'متوسط', advanced: 'متقدم' };
const LEVEL_DOT    = { basic: 'var(--success)', medium: 'var(--info)', advanced: 'var(--accent)' };
const STATUS_LABEL = { done: 'مكتمل', running: 'جارٍ التحليل', draft: 'مسودة' };

const StatusChip = ({ status }) => {
  if (status === 'done') return (
    <span className="chip chip--success"><IconCheck size={12}/> {STATUS_LABEL.done}</span>
  );
  ...
  return <span className="chip chip--neutral">{STATUS_LABEL.draft}</span>;
};
...
const LevelChip = ({ level }) => (
  <span className={`chip chip--${level}`}>
    <span style={{
      ...
      background: LEVEL_DOT[level], display: 'inline-block',
    }}/>
    {LEVEL_LABEL[level]}
  </span>
);
  ...
  return (
    <div className="card card--hover" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
        <h3 className="h3" style={{ margin: 0, flex: 1, fontSize: 18, lineHeight: 1.4 }}>
          {p.title}
        </h3>
        <button
        ...
        {menu && (
          ...
          >
            {[
              { icon: <IconEdit size={16}/>, label: 'تعديل' },
              { icon: <IconCopy size={16}/>, label: 'نسخ' },
              { icon: <IconTrash size={16}/>, label: 'حذف', danger: true },
            ].map(it => (
              <div key={it.label}
                style={{
                  ...
                  fontSize: 14, transition: 'background 150ms',
                }}
      ...
      <div style={{
        ...
        <span>أنشئ في <span className="num" style={{ color: 'var(--fg-secondary)' }}>{p.date}</span></span>
        <span>← فتح</span>
      </div>
    </div>
  );
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)' }}>
      ...
      <main style={{ maxWidth: 1280, margin: '0 auto', padding: '32px 32px 48px' }}>
        <div style={{
          ...
          <div>
            <h1 className="h-display" style={{ fontSize: 36, margin: 0 }}>مشاريعي</h1>
            <div style={{ marginTop: 6, color: 'var(--fg-secondary)', fontSize: 16 }}>
              كل تحليلاتك في مكان واحد
            </div>
          </div>
